...
const portraitFilter = [
  "crop=iw*0.8:ih*0.95:(iw-iw*0.8)/2:(ih-ih*0.95)/2",
  "scale=96:64:force_original_aspect_ratio=increase",
  "crop=96:64",
  "format=gray",
  "eq=contrast=1.5:brightness=0.08",
  "unsharp=5:5:1"
].join(",");

const profileLines = [
  { type: "header", value: "Nauval@dev" },
  { type: "row", key: "Subject", value: "Nauval Faiz Luthf Hisyam" },
  { type: "row", key: "Role", value: "Mobile Dev" },
  { type: "row", key: "Affiliation", value: "SMK Telkom Malang" },
  { type: "row", key: "Base", value: "Malang, Indonesia" },
  { type: "row", key: "Status", value: "Learning / Building / Testing" },
  { type: "blank" },
  { type: "section", value: "CURRENT.FOCUS" },
  { type: "row", key: "Primary", value: "QA Engineering" },
  { type: "row", key: "Direction", value: "Test Automation & Software Quality" },
  { type: "row", key: "Themes", value: "Flutter / Laravel / React" },
  { type: "blank" },
  { type: "section", value: "BUILD.LOG" },
  { type: "row", key: "MyFinance", value: "Personal Finance App" },
  { type: "row", key: "Laundry", value: "Laundry Management System" },
  { type: "row", key: "PDAM", value: "Mobile Service Application" },
  { type: "row", key: "Portfolio", value: "GitHub & Web Projects" },
  { type: "blank" },
  { type: "section", value: "GRID.LINKS" },
  { type: "row", key: "GitHub", value: "@NauvalFaiz" },
  { type: "footer", value: "FLUTTER / LARAVEL / QA ENGINEERING / IOT" }
];
...
const layouts = {
  desktop: {
    ...
    portrait: {
      columns: 118,      // sedikit lebih kecil
      rows: 80,
      x: 55,             // geser sedikit ke kanan
      y: 105,            // turun sekitar 50px
      lineHeight: 5.2,
      fontSize: 5.3
...
async function samplePortrait(sourcePath, columns, rows) {
  const { stdout } = await execFileAsync(
    ...
    [
      ...
      [
        "format=rgba",
        "scale=700:700:force_original_aspect_ratio=decrease",
        `pad=700:700:(ow-iw)/2:(oh-ih)/2:color=black@0`,
        "format=gray",
        `scale=${columns}:${rows}`,
        "eq=contrast=1.4:brightness=0.03",
        "unsharp=5:5:0.8"
      ].join(","),
...
function createAsciiTspans({ pixels, width, height }, placement) {
  ...
  for (let row = 0; row < height; row++) {
    ...
    for (let column = 0; column < width; column++) {
      ...
      const up = pixels[Math.max(row - 1, 0) * width + column];
      const down = pixels[Math.min(row + 1, height - 1) * width + column];

      // Gelap
      const darkness = 1 - pixel / 255;

      // Deteksi tepi
      const edge =
        (Math.abs(right - left) + Math.abs(down - up)) / 255;

      // Kontras
      let ink = darkness * 1.45 + edge * 0.30;
    ...
    rows.push(
      `<tspan
        x="${placement.x}"
        y="${(placement.y + row * placement.lineHeight).toFixed(2)}"
        xml:space="preserve">${escapeXml(line)}</tspan>`
    );
  }
